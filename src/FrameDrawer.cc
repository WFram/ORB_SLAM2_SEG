    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        if(!mvCurrentKeys.empty()){
            ...
            for(int i=0;i<nm;i++)
            {
                ...
                {
                    cv::Point2f pt1,pt2;
                    pt1.x=vCurrentMovKeys[i].pt.x-r;
                    pt1.y=vCurrentMovKeys[i].pt.y-r;
                    pt2.x=vCurrentMovKeys[i].pt.x+r;
                    pt2.y=vCurrentMovKeys[i].pt.y+r;
    
                    // This is a match to a MapPoint in the map
                    if(vbMap[i])
                    {
                        cv::rectangle(im,pt1,pt2,cv::Scalar(0,0,255));
                        cv::circle(im,vCurrentMovKeys[i].pt,2,cv::Scalar(0,0,255),-1);
                        mnTracked++;
                    }
                    else // This is match to a "visual odometry" MapPoint created in the last frame
                    {
                        cv::rectangle(im,pt1,pt2,cv::Scalar(255,0,0));
                        cv::circle(im,vCurrentMovKeys[i].pt,2,cv::Scalar(255,0,0),-1);
                        mnTrackedVO++;
                    }
                }
            }
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        if(!mvCurrentMovKeys.empty())
        {
            ...
            for(int i=0;i<nm;i++)
            {
                ...
                cv::rectangle(im,pt1,pt2,cv::Scalar(0,0,255));
                cv::circle(im,mvKFMovKeys[i].pt,2,cv::Scalar(0,0,255),-1);
            }
        ...
        if(!mvKFSegKeys.empty())
        {
            ...
            for(int i=0;i<nm;i++)
            {
                ...
                // This is a moving point in the KF
                cv::rectangle(im,pt1,pt2,cv::Scalar(255,0,0));
                cv::circle(im,mvKFSegKeys[i].pt,2,cv::Scalar(255,0,0),-1);
            }
